...
/**
 * WheelView滚轮控件，可以更新数据
 *
 * @author zhaozhijun
 */
public class WheelViewRefreshData extends View {
    /**
     * 控件宽度
     */
    ...
    public WheelViewRefreshData(Context context, AttributeSet attrs, int defStyle) {
        super(context, attrs, defStyle);
        init(context, attrs);
        initData();
    }

    public WheelViewRefreshData(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(context, attrs);
        initData();
    }

    public WheelViewRefreshData(Context context) {
        super(context);
        initData();
    }
    ...
    private void init(Context context, AttributeSet attrs) {
        lineColor = getResources().getColor(R.color.c3);
        normalSize = getResources().getDimension(R.dimen.dimen_16sp);
        selectedSize = getResources().getDimension(R.dimen.dimen_18sp);
        normalColor = getResources().getColor(R.color.hint_text_color);
        selectedColor = getResources().getColor(R.color.themeColor);
        unitHeight = (int) getResources().getDimension(R.dimen.dimen_40dp);
        ...
        normalSize = attribute.getDimension(R.styleable.WheelView_normalTextSize, normalSize);
        selectedSize = attribute.getDimension(R.styleable.WheelView_selectedTextSize, selectedSize);
        ...
        for (ItemObject item : itemList) {
            if (item.isSelected()) {
                if (onSelectListener != null)
                    onSelectListener.onSelect(item.id, item.itemText);
                break;
            }
        }
    }
        ...
        if (move > 0) {
            for (int i = 0; i < itemList.size(); i++) {
                if (itemList.get(i).isSelected()) {
                    ...
                    if (onSelectListener != null)
                        onSelectListener.onSelect(itemList.get(i).id,
...
//    }
    ...
    public void setData(List<String> data) {
        //设置新的数据，判断一下新的数据集是否包含旧的数据信
        //如果包含，可以不去移动现有选项
        int selectedIndex = getSelected();
        boolean contains = false;
        //在判断前要先设置旧数据源是否为空
        if (dataList.size() > 0 && data.containsAll(dataList)) {
            contains = true;
            for (int i = 0; i < data.size(); i++) {
                if (data.get(i).equals(getSelectedText())) {
                    //判断旧的选项在新的数据源中的对应位置
                    selectedIndex = i;
                    break;
                }
        ...
        setDefault(selectedIndex);
        if (onSelectListener != null) {
            onSelectListener.onSelect(selectedIndex, dataList.get(selectedIndex));
        }
    }
    ...
    private class ItemObject {
        ...
        public void drawSelf(Canvas canvas, int containerWidth) {
            ...
            if (isSelected()) {
                ...
                // 计算当前字体大小
                float textSize = normalSize
                        + ((selectedSize - normalSize) * (1.0f - moveToSelect / (float) unitHeight));
    ...
    public interface OnSelectListener {
         ...
         * @param text
         */
        void onSelect(int id, String text);
