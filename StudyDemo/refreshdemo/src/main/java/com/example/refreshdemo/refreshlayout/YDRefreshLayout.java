 ...
 */
public class YDRefreshLayout extends LinearLayout {
    private static final String TAG = YDRefreshLayout.class.getSimpleName();

    private YDRefreshViewHolder mRefreshViewHolder;
    /**
    ...
    private YDStickyNavLayout mStickyNavLayout;
    ...
    public YDRefreshLayout(Context context) {
        this(context, null);
    }

    public YDRefreshLayout(Context context, AttributeSet attrs) {
        super(context, attrs);
    ...
    public void onFinishInflate() {
        ...
        if (getChildCount() != 2) {
            throw new RuntimeException(YDRefreshLayout.class.getSimpleName() + "必须有且只有一个子控件");
        }
        ...
        } else if (mContentView instanceof YDStickyNavLayout) {
            mStickyNavLayout = (YDStickyNavLayout) mContentView;
    ...
    public void setRefreshViewHolder(YDRefreshViewHolder refreshViewHolder) {
    ...
    public boolean shouldHandleAbsListViewLoadingMore(AbsListView absListView) {
        ...
        return YDRefreshScrollingUtil.isAbsListViewToBottom(absListView);
    }
    ...
    public boolean shouldHandleRecyclerViewLoadingMore(RecyclerView recyclerView) {
        ...
        return YDRefreshScrollingUtil.isRecyclerViewToBottom(recyclerView);
    }
    ...
    private boolean shouldHandleLoadingMore() {
        ...
        if (YDRefreshScrollingUtil.isWebViewToBottom(mWebView)) {
            return true;
        }

        if (YDRefreshScrollingUtil.isScrollViewToBottom(mScrollView)) {
            return true;
        }
    ...
    private boolean isContentViewToTop() {
        ...
        if (YDRefreshScrollingUtil.isScrollViewOrWebViewToTop(mWebView)) {
            return true;
        }

        if (YDRefreshScrollingUtil.isScrollViewOrWebViewToTop(mScrollView)) {
            return true;
        }

        if (YDRefreshScrollingUtil.isAbsListViewToTop(mAbsListView)) {
            return true;
        }

        if (YDRefreshScrollingUtil.isRecyclerViewToTop(mRecyclerView)) {
            return true;
        }

        if (YDRefreshScrollingUtil.isStickyNavLayoutToTop(mStickyNavLayout)) {
            return true;
        }
    ...
    private void showLoadingMoreView() {
        ...
        YDRefreshScrollingUtil.scrollToBottom(mScrollView);
        YDRefreshScrollingUtil.scrollToBottom(mRecyclerView);
        YDRefreshScrollingUtil.scrollToBottom(mAbsListView);
        if (mStickyNavLayout != null) {
    ...
    public interface RefreshLayoutDelegate {
         ...
         * 开始刷新
         */
        void onRefreshLayoutBeginRefreshing(YDRefreshLayout refreshLayout);
         ...
         */
        boolean onRefreshLayoutBeginLoadingMore(YDRefreshLayout refreshLayout);
    }
